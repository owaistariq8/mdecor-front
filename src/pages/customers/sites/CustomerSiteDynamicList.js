...
export default function CustomerSiteDynamicList({ siteAddForm, siteEditForm, siteViewForm }) {
  ...
  const toggleChecked = () => {
    if (siteEditForm) {
      ...
      if(customerId ) navigate(PATH_CUSTOMERS.customers.sites.new(customerId))
    } else {
      if(customerId ) navigate(PATH_CUSTOMERS.customers.sites.new(customerId))
      dispatch(setCardActiveIndex(null));
      dispatch(setIsExpanded(false));
  ...
  useEffect(()=>{
    if( Array.isArray(sites) && sites?.length > 0 && customerId && !siteAddForm && !siteEditForm && !siteViewForm ){
      navigate(PATH_CUSTOMERS.customers.sites.view( customerId, sites[0]?._id))
    }
  ...
  const handleCardClick = async (_site)=>{
    if(customerId && _site._id ){
        await navigate(PATH_CUSTOMERS.customers.sites.view(customerId, _site?._id))
    }
}
    ...
    <Container maxWidth={ false }>
      <Grid container direction="row" justifyContent="space-between" alignItems="center" sx={{mb:2}}>
        <Grid item xs={12} md={6}>
          <BreadcrumbsProvider>
            <BreadcrumbsLink to={PATH_CUSTOMERS.customers.list} name={BREADCRUMBS.CUSTOMERS} />
            <BreadcrumbsLink to={PATH_CUSTOMERS.customers.view(customerId)} name={customer.name} />
            <BreadcrumbsLink
              to={PATH_CUSTOMERS.customers.sites.root(customerId)}
              name={
                <Stack>
