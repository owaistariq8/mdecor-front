...
export default function SiteEditForm() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if(customerId && id ){
        ...
        await navigate(PATH_CUSTOMERS.customers.sites.view(customerId, id))
      } 
      await reset();
    } catch (err) {
  ...
  const toggleCancel = () => {if(customerId && id ){ navigate(PATH_CUSTOMERS.customers.sites.view(customerId, id))}}
