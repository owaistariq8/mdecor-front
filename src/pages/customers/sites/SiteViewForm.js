...
export default function SiteViewForm({ handleMap }) {
  ...
  const onDelete = async () => {
    try {
      ...
      if(customerId ) await navigate(PATH_CUSTOMERS.customers.sites.root( customerId ))
    } catch (err) {
  ...
  const handleEdit = async () => { if(customerId && id ) navigate(PATH_CUSTOMERS.customers.sites.edit(customerId, id))};
